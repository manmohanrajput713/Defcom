...
import { showMessageNotification, requestNotificationPermissions } from '@/lib/notifications';
...
export default function ChatsScreen() {
        ...
        .on(
          ...
          async (payload) => {
            ...
            if (
              ...
              !previousMessageIds.current.has(newMessage.id) &&
              ...
              console.log('[notifications] Showing background notification for:', senderName);
              await showMessageNotification(senderName, messageContent, newMessage.conversation_id);
            }
            loadConversations();
          }
        )
  ...
  const renderConversation = ({ item }: { item: Conversation }) => {
    ...
    const avatarSource = getAvatarSource(item.other_user.avatar_url);
        ...
        <View style={styles.conversationContent}>
          ...
          {item.last_message && (
            <Text style={[
              ...
              item.unread_count > 0 && styles.conversationMessageUnread
            ]} numberOfLines={1}>
              {(() => {
                const content = item.last_message.content;
                // Check if message looks encrypted (JSON format) or is a placeholder
                if (!content || content.startsWith('{') || content.startsWith('🔒') || content === '[encrypted]' || content === '[destroyed]') {
                  return '💬 New message';
                }
                return content;
              })()}
    ...
    <View style={styles.container}>
      <View style={styles.header}>
        ...
        <TouchableOpacity
          ...
          onPress={() => setShowNewChat(true)}
        >
          <Plus size={22} color="#4A7C59" />
        </TouchableOpacity>
      </View>

      {showNewChat && (
        <View style={styles.newChatContainer}>
          <Text style={styles.newChatTitle}>NEW CHANNEL</Text>
          ...
          <View style={styles.buttonRow}>
            ...
            <TouchableOpacity style={styles.createButton} onPress={createNewChat}>
              <Text style={styles.createButtonText}>INITIATE</Text>
            </TouchableOpacity>
          </View>
        </View>
      )}
      ...
      {conversations.length === 0 ? (
        <View style={styles.emptyContainer}>
          <View style={styles.emptyIconCircle}>
            <MessageCircle size={40} color="#4A7C59" />
          </View>
          <Text style={styles.emptyText}>NO ACTIVE CHANNELS</Text>
          <Text style={styles.emptySubtext}>
            Tap + to initiate secure communication
          </Text>
        </View>
      ) : (
        <FlatList
