...
export function crypto_sign_ed25519_pk_to_curve25519(ed25519PublicKey: Uint8Array): Uint8Array {
  ...
  console.log('[crypto_sign_ed25519_pk_to_curve25519] Available methods:', Object.keys(nobleEd25519));
  console.log('[crypto_sign_ed25519_pk_to_curve25519] Utils available:', nobleEd25519.utils ? Object.keys(nobleEd25519.utils) : 'utils is undefined');
  
  if (nobleEd25519.utils && nobleEd25519.utils.toMontgomery) {
    const converted = nobleEd25519.utils.toMontgomery(ed25519PublicKey);
    console.log('[crypto_sign_ed25519_pk_to_curve25519] Converted public key successfully');
    console.log('[crypto_sign_ed25519_pk_to_curve25519] Key preview:', Array.from(converted.slice(0, 8)).map(b => b.toString(16).padStart(2, '0')).join(' '));
    console.log('[crypto_sign_ed25519_pk_to_curve25519] Key all zeros:', converted.every(byte => byte === 0));
    return converted;
  } else {
    throw new Error('toMontgomery function not available in nobleEd25519.utils');
  }
}
...
export function crypto_sign_ed25519_sk_to_curve25519(ed25519SecretKey: Uint8Array): Uint8Array {
  ...
  console.log('[crypto_sign_ed25519_sk_to_curve25519] Available methods:', Object.keys(nobleEd25519));
  console.log('[crypto_sign_ed25519_sk_to_curve25519] Utils available:', nobleEd25519.utils ? Object.keys(nobleEd25519.utils) : 'utils is undefined');
  
  // Use the exact libsodium implementation for Ed25519 to Curve25519 private key conversion
  // This is the standard RFC 8032 method
  const hash = new SHA512();
  hash.update(seed);
  const hashedSeed = hash.digest();
  
  // Take only the first 32 bytes (SHA-512 produces 64 bytes)
  const curve25519PrivateKey = hashedSeed.slice(0, 32);
  
  // Apply Curve25519 clamping (this is critical for compatibility)
  curve25519PrivateKey[0] &= 248;  // Clear the 3 least significant bits
  curve25519PrivateKey[31] &= 127; // Clear the most significant bit  
  curve25519PrivateKey[31] |= 64;  // Set the second most significant bit
  
  console.log('[crypto_sign_ed25519_sk_to_curve25519] Converted private key successfully using SHA-512');
  console.log('[crypto_sign_ed25519_sk_to_curve25519] Key preview:', Array.from(curve25519PrivateKey.slice(0, 8)).map(b => b.toString(16).padStart(2, '0')).join(' '));
  console.log('[crypto_sign_ed25519_sk_to_curve25519] Key all zeros:', curve25519PrivateKey.every(byte => byte === 0));
