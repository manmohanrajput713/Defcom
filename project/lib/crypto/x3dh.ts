...
export async function generateDeviceKeys(
  ...
  console.log(`[generateDeviceKeys] Starting key generation for device: ${deviceLabel}`);
  
  try {
    const sodium = await getSodium();
    console.log('[generateDeviceKeys] Sodium module loaded successfully');
    
    const signingKeyPair = sodium.crypto_sign_keypair();
    console.log('[generateDeviceKeys] Ed25519 signing key pair generated');
    
    const identityPublic = sodium.crypto_sign_ed25519_pk_to_curve25519(
      signingKeyPair.publicKey
    );
    console.log('[generateDeviceKeys] Ed25519 public key converted to Curve25519, length:', identityPublic.length);
    
    const identityPrivate = sodium.crypto_sign_ed25519_sk_to_curve25519(
      signingKeyPair.privateKey
    );
    console.log('[generateDeviceKeys] Ed25519 private key converted to Curve25519, length:', identityPrivate.length);
    
    const signedPreKeyPair = sodium.crypto_kx_keypair();
    console.log('[generateDeviceKeys] Signed prekey pair generated');
    
    const oneTimePreKeys = toOneTimePreKeys(sodium, oneTimePreKeyCount);
    console.log(`[generateDeviceKeys] Generated ${oneTimePreKeys.length} one-time prekeys`);
    ...
    console.log('[generateDeviceKeys] Signed prekey signature created');
    ...
    console.log('[generateDeviceKeys] Device keys generated successfully ✅');
    return { ...storedKeys, bundle };
    
  } catch (error) {
    console.error('[generateDeviceKeys] Error during key generation:', error);
    throw error;
  }
}
...
export async function performX3DHInitiation({
  ...
  console.log('[performX3DHInitiation] Starting X3DH initiation...');
  const sodium = await getSodium();
  ...
  console.log('[performX3DHInitiation] Identity private key length:', identityPrivate.length);
  ...
  console.log('[performX3DHInitiation] Computing shared secrets...');
  ...
  // Validate that scalar multiplication produced valid results
  console.log('[performX3DHInitiation] Scalar multiplication results:', {
    contribution1Length: contributions[0].length,
    contribution1Preview: Array.from(contributions[0].slice(0, 8)).map(b => b.toString(16).padStart(2, '0')).join(' '),
    contribution1AllZeros: contributions[0].every(byte => byte === 0),
    contribution2Length: contributions[1].length,
    contribution2Preview: Array.from(contributions[1].slice(0, 8)).map(b => b.toString(16).padStart(2, '0')).join(' '),
    contribution2AllZeros: contributions[1].every(byte => byte === 0),
    contribution3Length: contributions[2].length,
    contribution3Preview: Array.from(contributions[2].slice(0, 8)).map(b => b.toString(16).padStart(2, '0')).join(' '),
    contribution3AllZeros: contributions[2].every(byte => byte === 0)
  });
  ...
  if (remoteOneTimePreKey?.publicKey) {
    ...
    console.log('[performX3DHInitiation] Added one-time prekey contribution');
  }

  const sharedSecret = concat(contributions);
  console.log('[performX3DHInitiation] Shared secret computed, length:', sharedSecret.length);
  console.log('[performX3DHInitiation] Shared secret preview:', Array.from(sharedSecret.slice(0, 8)).map(b => b.toString(16).padStart(2, '0')).join(' '));
  console.log('[performX3DHInitiation] Shared secret all zeros:', sharedSecret.every(byte => byte === 0));
  
  const secrets = await deriveInitialSecrets(sharedSecret);
  console.log('[performX3DHInitiation] Initial secrets derived');
  ...
  console.log('[performX3DHInitiation] X3DH initiation completed successfully');
  return {
    ratchetState,
    ephemeralKeyPair,
    usedOneTimePreKey: remoteOneTimePreKey,
...
export async function completeX3DHResponse({
  ...
  console.log('[completeX3DHResponse] Starting X3DH response...');
  const sodium = await getSodium();
  const signedPreKeyPrivate = decode(sodium, localKeys.signedPreKey.privateKey);
  const identityPrivate = decode(sodium, localKeys.identityKey.privateKey);
  console.log('[completeX3DHResponse] Identity private key length:', identityPrivate.length);
  
  const initiatorIdentity = decode(sodium, initiatorIdentityKey);
  const initiatorEphemeral = decode(sodium, initiatorEphemeralKey);

  console.log('[completeX3DHResponse] Computing shared secrets...');
  const contributions: Uint8Array[] = [
  ...
  // Validate that scalar multiplication produced valid results
  console.log('[completeX3DHResponse] Scalar multiplication results:', {
    contribution1Length: contributions[0].length,
    contribution1Preview: Array.from(contributions[0].slice(0, 8)).map(b => b.toString(16).padStart(2, '0')).join(' '),
    contribution1AllZeros: contributions[0].every(byte => byte === 0),
    contribution2Length: contributions[1].length,
  ...
  if (usedOneTimePreKey) {
    ...
    if (consumedOneTimePreKey) {
      ...
      console.log('[completeX3DHResponse] Added one-time prekey contribution');
    } else if (__DEV__) {
  ...
  const sharedSecret = concat(contributions);
  console.log('[completeX3DHResponse] Shared secret computed, length:', sharedSecret.length);
  console.log('[completeX3DHResponse] Shared secret preview:', Array.from(sharedSecret.slice(0, 8)).map(b => b.toString(16).padStart(2, '0')).join(' '));
  console.log('[completeX3DHResponse] Shared secret all zeros:', sharedSecret.every(byte => byte === 0));
  
  const secrets = await deriveInitialSecrets(sharedSecret);
  console.log('[completeX3DHResponse] Initial secrets derived');
  ...
  console.log('[completeX3DHResponse] X3DH response completed successfully');
  return {
    ratchetState,
