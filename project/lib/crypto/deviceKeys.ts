...
export async function ensureDeviceKeys(
  ...
  console.log(`[ensureDeviceKeys] Starting device key generation for user: ${userId}`);
  ...
  if (!keys || keys.deviceLabel !== deviceLabel) {
    console.log('[ensureDeviceKeys] Generating new device keys...');
    try {
      ...
      console.log('[ensureDeviceKeys] Device keys generated and saved successfully');
    } catch (error) {
      console.error('[ensureDeviceKeys] Error generating device keys:', error);
      throw error;
    }
  ...
  console.log('[ensureDeviceKeys] Publishing device keys to server...');
  await publishDeviceKeys(userId, keys);
  console.log('[ensureDeviceKeys] Device keys setup completed successfully');
  return keys;
}
