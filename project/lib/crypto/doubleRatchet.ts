...
const kdfRoot = async (
  ...
  console.log('[DoubleRatchet] kdfRoot starting with rootKey length:', rootKey.length, 'dhOutput length:', dhOutput.length);
  const input = concat(rootKey, dhOutput);
  const derived = await hkdf(input, 'DoubleRatchetRoot', 64);
  console.log('[DoubleRatchet] kdfRoot derived length:', derived.length);
  return {
...
const kdfChain = async (
  ...
  console.log('[DoubleRatchet] kdfChain starting with chainKey length:', chainKey.length);
  const derived = await hkdf(chainKey, 'DoubleRatchetChain', 64);
  console.log('[DoubleRatchet] kdfChain derived length:', derived.length);
  return {
...
export class DoubleRatchet {
  ...
  static async fromState(state: RatchetState): Promise<DoubleRatchet> {
    console.log('[DoubleRatchet] fromState called with state:', {
      rootKeyLength: state.rootKey.length,
      sendingChainKeyLength: state.sendingChainKey.length,
      receivingChainKeyLength: state.receivingChainKey.length,
      sendingMessageNumber: state.sendingMessageNumber,
      receivingMessageNumber: state.receivingMessageNumber,
      hasLocalRatchetKey: !!state.localRatchetKey,
      hasRemoteRatchetKey: !!state.remoteRatchetKey
    });
    
    const sodium = await getSodium();
    const runtime = decodeState(sodium, state);
    
    console.log('[DoubleRatchet] decoded runtime state:', {
      rootKeyLength: runtime.rootKey.length,
      sendingChainKeyLength: runtime.sendingChainKey.length,
      receivingChainKeyLength: runtime.receivingChainKey.length,
      localRatchetKeyLength: runtime.localRatchetKey.publicKey.length,
      hasRemoteRatchetKey: !!runtime.remoteRatchetKey,
      receivingChainKeyPreview: Array.from(runtime.receivingChainKey.slice(0, 8)).map(b => b.toString(16).padStart(2, '0')).join(' '),
      receivingChainKeyAllZeros: runtime.receivingChainKey.every(byte => byte === 0)
    });
  ...
  async encrypt(
    ...
    const ciphertext = this.sodium.crypto_aead_xchacha20poly1305_ietf_encrypt(
      plaintextBytes,
      adBytes,
      null, // _secretNonce should be null
      nonce,
      messageKey
    );
  ...
  private decryptWithKey(
    ...
    console.log('[DoubleRatchet] decryptWithKey starting...', {
    ...
    if (!plaintext) {
      console.error('[DoubleRatchet] decryptWithKey failed - decryption returned null');
      throw new Error('Decryption failed: invalid ciphertext or key');
    }

    console.log('[DoubleRatchet] decryptWithKey successful');
  ...
  async decryptIncoming(
    ...
    const currentMessageNumber = this.state.receivingMessageNumber;
    const { chainKey, messageKey } = await kdfChain(this.state.receivingChainKey);
    console.log('[DoubleRatchet] decryptIncoming derived keys:', {
      chainKeyLength: chainKey.length,
      messageKeyLength: messageKey.length,
      messageKeyPreview: Array.from(messageKey.slice(0, 8)).map(b => b.toString(16).padStart(2, '0')).join(' ')
    });
    ...
    console.log('[DoubleRatchet] decryptIncoming decoded data:', {
      nonceLength: nonce.length,
      ciphertextLength: ciphertext.length,
      ciphertextPreview: Array.from(ciphertext.slice(0, 8)).map(b => b.toString(16).padStart(2, '0')).join(' '),
    ...
    try {
      ...
      console.error('[DoubleRatchet] decryptIncoming failed:', {
  ...
  async decryptWithKnownKey(
    ...
    console.log('[DoubleRatchet] decryptWithKnownKey starting...');
    ...
    try {
      ...
      console.error('[DoubleRatchet] decryptWithKnownKey failed:', {
        error: error instanceof Error ? error.message : error,
        stack: error instanceof Error ? error.stack : undefined,
        header: payload.header,
        ciphertextLength: payload.ciphertext.length,
